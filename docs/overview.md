...
`inventory.transactions` を継続的に読み込み、SKU とブランチでアグリゲーションし、最新の在庫状況を `inventory.availability` に書き込みます。
...
| エージェント名 | 役割 |
|--------------|------|
| **SKU_Availability_Agent** | 在庫確認ツールを呼び出して、指定ブランチの在庫をチェックする |
| **Substitute_Finder_Agent** | 要求された SKU が在庫切れの場合、エンタープライズ製品ドキュメントから類似 SKU を推薦する |
| **Store_Associate_Agent** | 上記 2 つの専門エージェントに委任し、顧客向けの回答をまとめるスーパーバイザーエージェント |

### エンタープライズドキュメント

製品カタログドキュメント（`product-catalog.docx`）をエンタープライズナレッジソース（`enterprise_documents`）としてアップロードし、Substitute Finder Agent でのセマンティック類似検索に使用します。
...
│                                                                 │
│  ┌─────────────────────────┐    ┌──────────────────────────┐   │
│  │  inventory.transactions  │───▶│  ksqlDB (stream proc.)   │   │
│  │  (source topic)          │    │  SUM(quantity)           │   │
│  └─────────────────────────┘    │  GROUP BY sku, branch    │   │
│                                  └───────────┬──────────────┘   │
│                                              │                  │
│                                  ┌───────────▼──────────────┐   │
...
└─────────────────────────────────────────────────────────────────┘
                                              │
                              ┌───────────────▼───────────────┐
                              │   Kafka Availability Tool     │
                              │   (MCP Toolkit)               │
                              │   get_sku_availability(sku,   │
                              │                      branch)  │
                              └───────────────┬───────────────┘
                                              │
┌─────────────────────────────────────────────▼────────────────────┐
...
│                                                                   │
│  ┌──────────────────────────────────────────────────────────┐    │
│  │              Store_Associate_Agent (Supervisor)           │    │
│  │                                                           │    │
│  │  ┌──────────────────────┐  ┌────────────────────────┐   │    │
│  │  │  SKU_Availability    │  │  Substitute_Finder     │   │    │
│  │  │  Agent               │  │  Agent (Agentic RAG)   │   │    │
│  │  └──────────────────────┘  └────────────┬───────────┘   │    │
│  └──────────────────────────────────────────┼───────────────┘    │
└─────────────────────────────────────────────┼────────────────────┘
                                              │
