...
エージェントの作成・デプロイが完了した時点では、まだ製品カタログドキュメントへのアクセス権がありません。<br>
次の手順で製品カタログをナレッジソースとして登録します。

### 製品カタログについて

このハンズオンでは、内部製品カタログを表す **1 つの Word ドキュメント**（`product-catalog.docx`）を使用します。<br>
このドキュメントには複数の製品エントリが一貫したフォーマットで記載されており、セマンティック検索と類似マッチングに適しています。

ステップ4でダウンロードしたファイルの中に `product-catalog.docx` があります。
...
### テスト2
```
LAPTOP-DELL-XPS-15 は在庫がありません。
製品カタログを使って類似のラップトップを提案してください。
```
**期待される動作：**  
